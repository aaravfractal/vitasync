...
## Current state
**The client store is the source of truth.** Every button works against `src/lib/store.tsx` (React context + localStorage) seeded from `src/lib/demo-data.ts`. Bookings, logged vitals, refill orders, access grants/revokes, family/caregiver, reminders, language and sign-out all persist across refresh. `clickthrough.py` (Playwright) exercises every button; keep it green. It stays this way until a clinic partner is signed — no real patient data exists yet, so a database would only add operational risk and a privacy surface we do not need. Supabase (step 1) is deferred until then.
...
`/api/chat` uses Claude when `ANTHROPIC_API_KEY` is set, mock otherwise, capped at 5 requests per IP per day (`src/lib/rate-limit.ts`, in-memory); over the cap it returns 429 and the chat shows "Daily limit reached. Plus gets unlimited." with a link to `/pricing`. `/api/reports/summary` follows the same Claude-or-mock pattern. Emergency uses `navigator.geolocation` and Overpass (`amenity=hospital` + `emergency=yes`, 10 km) on the device, falling back to the seeded Dehradun list if it is denied, fails, or takes over 4 s.
